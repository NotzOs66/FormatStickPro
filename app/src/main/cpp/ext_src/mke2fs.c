...
// Redirecționăm exit() către funcția noastră de salvare
extern void mke2fs_exit(int status);
#define exit(s) mke2fs_exit(s)
...
static void write_inode_tables(ext2_filsys fs, int lazy_flag, int itable_zeroed)
{
    ...
    if (strcmp(device_name, "usb_device") == 0) lazy_flag = 1;
		if (lazy_flag)
...
static void create_root_dir(ext2_filsys fs)
{
	...
	// FORCE FULL PERMISSIONS (777) for Android/Linux sticks
	// This way anyone can write on it without sudo chown
	if (root_perms == (mode_t)-1) root_perms = 0777;
...
profile_error:
	...
	if (strcmp(device_name, "usb_device") == 0) {
		io_ptr = usb_scsi_io_manager;
	}
...
void figure_journal_size(struct ext2fs_journal_params *jparams,
				int journal_size, int journal_fc_size,
				ext2_filsys fs) {
    blk64_t j_blocks;
    if (journal_size > 0) {
        j_blocks = (blk64_t)journal_size * 1024 / fs->blocksize;
    } else {
        // Default small journal for removable sticks: 32MB
        j_blocks = (32 * 1024 * 1024) / fs->blocksize;
    }
    jparams->num_journal_blocks = (unsigned int)j_blocks;
    jparams->num_fc_blocks = (journal_fc_size > 0) ? (unsigned int)((journal_fc_size * 1024) / fs->blocksize) : 0;
}

void print_check_message(int max_mnt_count, unsigned int checkinterval) {
    (void)max_mnt_count; (void)checkinterval;
}
