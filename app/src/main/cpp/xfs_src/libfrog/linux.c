...
platform_findsizes(char *path, int fd, long long *sz, int *bsz)
{
	...
	if (fd == -200 || is_usb_dev(path)) {
		// total_sectors from bridge should be passed here, but how?
		// In f2fs we used a global. Let's do the same or check if mkfs passed it.
		// Actually mkfs.xfs tries to detect size if not provided.
		// For now let's assume we use a global.
		extern uint64_t g_usb_scsi_part_sectors;
		*sz = (long long)g_usb_scsi_part_sectors;
		*bsz = 512;
		return;
	}
