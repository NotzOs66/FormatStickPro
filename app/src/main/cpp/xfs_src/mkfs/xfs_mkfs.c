...
static void
reset_mkfs_state(void)
{
	printf("MKFS-DEBUG: Entering reset_mkfs_state\n");
	reset_opts(&bopts);
	reset_opts(&copts);
	reset_opts(&dopts);
	reset_opts(&iopts);
	...
	struct cli_params *cli)
{
	...
	printf("MKFS-DEBUG: Entering parse_subopts, opt='%c', arg='%s'\n", opt, arg);
	...
	/* should never happen */
	if (!sop->opts) {
		printf("MKFS-DEBUG: parse_subopts: option '%c' not found in table!\n", opt);
		return;
	}
	...
	while (p && *p != '\0') {
		...
		printf("MKFS-DEBUG: parse_subopts: calling getsubopt, p='%s'\n", p);
		subopt = getsubopt(&p, subopts, &value);
		printf("MKFS-DEBUG: parse_subopts: getsubopt returned %d, value='%s', next p='%s'\n",
				subopt, value ? value : "NULL", p ? p : "NULL");
...
static struct libxfs_init	xi;
static struct xfs_mount	mbuf;
static struct cli_params	cli;
static struct mkfs_params	cfg;
static struct mkfs_default_params dft;
static struct fs_topology	ft;
static struct zone_topology zt;
...
xfs_mkfs_main(
	...
	char			**argv)
{
	printf("MKFS-DEBUG: Entering xfs_mkfs_main, argc=%d\n", argc);
	xfs_agnumber_t		agno;
	struct xfs_buf		*buf;
	int			c;
	int			dry_run = 0;
	...
	printf("MKFS-DEBUG: Calling reset_mkfs_state\n");
	reset_mkfs_state();

	printf("MKFS-DEBUG: Initializing structures\n");
	memset(&xi, 0, sizeof(xi));
	xi.flags = LIBXFS_EXCLUSIVELY | LIBXFS_DIRECT;
	...
	dft.source = (char*)_("package build definitions");
	dft.sectorsize = XFS_MIN_SECTORSIZE;
	dft.blocksize = 1 << XFS_DFL_BLOCKSIZE_LOG;
	dft.sb_feat.log_version = 2;
	dft.sb_feat.attr_version = 2;
	dft.sb_feat.dir_version = 2;
	dft.sb_feat.inode_align = true;
	dft.sb_feat.lazy_sb_counters = true;
	dft.sb_feat.projid32bit = true;
	dft.sb_feat.crcs_enabled = true;
	dft.sb_feat.dirftype = true;
	dft.sb_feat.finobt = true;
	dft.sb_feat.spinodes = true;
	dft.sb_feat.rmapbt = true;
	dft.sb_feat.reflink = true;
	dft.sb_feat.inobtcnt = true;
	dft.sb_feat.parent_pointers = true;
	dft.sb_feat.bigtime = true;
	dft.sb_feat.nrext64 = true;
	dft.sb_feat.exchrange = true;

	memset(&zt, 0, sizeof(zt));
	struct list_head	buffer_list;
	int			error;

	printf("MKFS-DEBUG: Calling platform_uuid_generate\n");
	platform_uuid_generate(&cli.uuid);
	printf("MKFS-DEBUG: Calling basename\n");
	progname = basename(argv[0]);
	printf("MKFS-DEBUG: progname=%s\n", progname);
	printf("MKFS-DEBUG: About to copy sb_feat\n");
	/* copy new defaults into CLI parsing structure */
	memcpy(&cli.sb_feat, &dft.sb_feat, sizeof(cli.sb_feat));
	printf("MKFS-DEBUG: About to copy fsx\n");
	memcpy(&cli.fsx, &dft.fsx, sizeof(cli.fsx));

	printf("MKFS-DEBUG: Starting getopt loop, argc=%d\n", argc);
	while ((c = getopt_long(argc, argv, "b:c:d:i:l:L:m:n:KNp:qr:s:CfV",
					long_options, &option_index)) != EOF) {
		printf("MKFS-DEBUG: getopt returned %c (%d)\n", c, c);
		switch (c) {
		case 0:
			break;
		case 'C':
