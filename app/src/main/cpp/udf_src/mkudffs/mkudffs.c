...
#include "usb_scsi.h"
extern ssize_t udf_read_nointr(int fd, void *buf, size_t count);
extern ssize_t udf_write_nointr(int fd, const void *buf, size_t count);

#define read_nointr udf_read_nointr
#define write_nointr udf_write_nointr

extern void udf_msg(const char *fmt, ...);
#define MSG(n, fmt, ...) udf_msg(fmt, ##__VA_ARGS__)
