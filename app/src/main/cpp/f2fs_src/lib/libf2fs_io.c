...
#if defined(WITH_ANDROID) && !defined(ANDROID)
...
{
	if (c.devices[0].path && strcmp(c.devices[0].path, "usb_device") == 0) {
		if (usb_scsi_pread_internal(buf, (int64_t)len, (int64_t)offset) == (int64_t)len) return 0;
		return -1;
	}
	int fd;
	int err;
...
{
	if (c.devices[0].path && strcmp(c.devices[0].path, "usb_device") == 0) {
		if (usb_scsi_pwrite_internal(buf, (int64_t)len, (int64_t)offset) == (int64_t)len) return 0;
		return -1;
	}
	int fd;
...
int f2fs_finalize_device(void)
{
	 ...
	 */
	for (i = 0; i < c.ndevs; i++) {
		if (c.devices[i].path && strcmp(c.devices[i].path, "usb_device") == 0) {
