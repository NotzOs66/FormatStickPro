...
int get_device_info(int i)
{
	...
	if (strcmp(dev->path, "usb_device") == 0) {
		memset(stat_buf, 0, sizeof(struct stat));
		stat_buf->st_mode = S_IFBLK;
		fd = 0; // Not used for usb_device in dev_read/write
	} else if (!c.sparse_mode) {
	...
	if (strcmp(dev->path, "usb_device") == 0) {
		dev->total_sectors = c.wanted_total_sectors;
		c.kd = -2; // No kernel support
		c.start_sector = 0;
		if (!c.sector_size) {
			c.sector_size = 512;
			c.sectors_per_blk = 8;
